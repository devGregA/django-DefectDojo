...
DefectDojo open-source moved back to the Authorized Users model in version 2.58. If you're upgrading from a release that had the Members / Groups / Global Roles system, your existing access is carried forward into Authorized Users automatically by the upgrade — no manual mapping is needed.

The upgrade ships with a read-only management command, `preview_legacy_authorization_migration`, that summarizes what an upgrade would change against a copy of your database. The recommended workflow is to install 2.58 in a staging environment with a snapshot of production, run the command, review the summary, and then upgrade production.
...
For more detail on both paths, see the [2.58 upgrade notes](/releases/os_upgrading/2.58/#authorized-users-panel-replaces-membersgroups-under-legacy-authorization).
